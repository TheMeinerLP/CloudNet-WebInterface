...
public class ProxyConfigJsonAdapter implements JsonSerializer<ProxyConfig>,
    ...
    public ProxyConfig deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext
        ...
        List<Motd> motdsLayouts = new ArrayList<>();
        object.get("motdsLayouts").getAsJsonArray()
                .forEach(t -> motdsLayouts.add(jsonDeserializationContext
                        .deserialize(t, Motd.class)));
        Motd maintenanceMotdLayout = jsonDeserializationContext
                .deserialize(object.get("maintenanceMotdLayout"),
                        Motd.class);
        String maintenaceProtocol = object.get("maintenaceProtocol").getAsString();
        int maxPlayers = object.get("maxPlayers").getAsInt();
        boolean customPayloadFixer = object.get("customPayloadFixer").getAsBoolean();
        AutoSlot autoSlot = jsonDeserializationContext
                .deserialize(object.get("autoSlot"), AutoSlot.class);
        TabList tabList = jsonDeserializationContext
        ...
        DynamicFallback dynamicFallback = jsonDeserializationContext
        ...
        return new ProxyConfig(enabled, maintenance, motdsLayouts, maintenanceMotdLayout,
                ...
                customPayloadFixer, autoSlot, tabList, playerInfos.toArray(new String[0]),
