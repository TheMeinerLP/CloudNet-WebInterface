...
public class SignLayoutConfigAdapter implements JsonSerializer<SignLayoutConfig>,
    JsonDeserializer<SignLayoutConfig> {

  @Override
  public SignLayoutConfig deserialize(JsonElement jsonElement, Type type,
    ...
    object.get("groupLayouts").getAsJsonArray().forEach(
        t -> groupLayouts.add(jsonDeserializationContext.deserialize(t, SignGroupLayouts.class)));
    SearchingAnimation searchingAnimation = jsonDeserializationContext
        .deserialize(object.get("searchingAnimation"), SearchingAnimation.class);
    return new SignLayoutConfig(fullServerHide, knockbackOnSmallDistance, distance, strength,
        groupLayouts, searchingAnimation);
  }

  @Override
  public JsonElement serialize(SignLayoutConfig signLayoutConfig, Type type,
    ...
    signLayoutConfig.getGroupLayouts()
        .forEach(t -> groupLayouts.add(jsonSerializationContext.serialize(t)));
    object.add("groupLayouts", groupLayouts);
    object.add("searchingAnimation",
        jsonSerializationContext.serialize(signLayoutConfig.getSearchingAnimation()));
    return object;
  }
}
